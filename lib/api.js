...
export const api = {
  ...
  async getSeasonsData() {
    const data = await apiRequest('/api/seasons/data');
    // API route returns array directly from Next.js route
    // Backend returns { success: true, seasons: [...] }
    // Frontend API route extracts seasons array
    if (Array.isArray(data)) {
      return { seasons: data };
    }
    // Handle case where backend response is passed through
  ...
  async getSeason3Data() {
    return apiRequest('/api/seasons/data');
  },
  ...
  // Roster management
  async getRoster() {
    return apiRequest('/api/roster');
  },

  async saveRoster(rosterData, credentials) {
  ...
  async getRankings() {
    return apiRequest('/api/rankings/all')
  },

  async getMemberEnrichment(realm, name) {
    return apiRequest(`/api/rankings/member/${realm}/${name}`)
  },
};
